...
billing_urls = [url(r'^bill_review', v.bill_review),
                url(r'^bill_delay', v.bill_payment_delay),
                url(r'^bill/(?P<bill_id>\d+)/mark_bill_paid$', v.mark_bill_paid),
                url(r'^file/(?P<nature>.+)/(?P<bill_id>\d+)$', v.bill_file),
                url(r'^pdf/(?P<bill_id>\d+)$', v.BillPdf.as_view(), name="bill_pdf"),
                url(r'^html/(?P<bill_id>\d+)$', v.Bill.as_view()),
                ...
                url(r'^pre_billing$', v.pre_billing, {"mine": False, }),
                url(r'^pre_billing/mine$', v.pre_billing, {"mine": True, }),
                url(r'^pre_billing/(?P<year>\d+)/(?P<month>\d+)/$', v.pre_billing, {"mine": False, }),
                url(r'^pre_billing/(?P<year>\d+)/(?P<month>\d+)/mine$', v.pre_billing, {"mine": True, }),
                ...
                url(r'^graph/billing-jqp$', v.graph_billing_jqp),
                url(r'^graph/yearly-billing$', v.graph_yearly_billing),
                ]

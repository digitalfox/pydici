...
class ActivityQuerySet(models.QuerySet):
    ...
    def late(self):
        return self.filter(Q(due_date__lt=date.today()) | Q(due_date__isnull=True)).filter(state="TODO_PLANNED")

    def soon(self):
        return self.filter(due_date__lt=(date.today() + timedelta(days=30)), due_date__gte=date.today(), state="TODO_PLANNED")

    def later(self):
        return self.filter(due_date__gt=(date.today() + timedelta(days=30))).filter(state="TODO_PLANNED")
